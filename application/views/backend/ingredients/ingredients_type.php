<div class="content-wrapper">
    ...
    <section class="content-header">
      <h1>
        <b>ข้อมูลประเภทส่วนผสม</b>
        <small></small>
      </h1>
      <ol class="breadcrumb">
        <li><a href="#"><i class="fa fa-dashboard"></i> หน้าหลัก</a></li>
        <li class="active">ประเภทส่วนผสมทั้งหมด</li>
      </ol>
    </section>
          ...
          <div class="col-xs-12">
            <div class="box">
              <div class="box-header">
                <h3 class="box-title"><b>ข้อมูลประเภทส่วนผสม</b></h3>
              </div>
              <!-- /.box-header -->
              <div class="box-body">
                      ...
                      <div class="btn-toolbar" role="toolbar" aria-label="Toolbar with button groups">
                        <div class="btn-group" role="group" aria-label="1 group">
                          <a href ="" class="btn btn-sm btn-success btn-flat box-add" title="เพิ่มประเภทของผลิตภัณฑ์" data-toggle="modal" 
                            ...
                            <i class="fa fa-plus"></i> เพิ่มประเภทส่วนผสม
                          </a>
                        </div>
                ...
                <table id="example1" class="table table-bordered table-striped">
                  <thead>
                    <tr class="css_tr_data">
                        ...
                        <th width='35%'>ประเภทของส่วนผสม</th>
                        <th width='5%'>แก้ไข</th>
                        <th width='10%'>ลบประเภทของส่วนผสม</th>
                    </tr>
                  </thead>
                  <tbody>
                    <?php foreach ($ingredients_type as $rw) { ?>
                      <tr class="css_tr_data">
                        <td>
                          <input name="checkdel[]" type="checkbox" class="css_data_item" id="<?php echo $rw->i_t_id; ?>" value="<?php echo $rw->i_t_id; ?>">
                        </td>
                        <td><?php echo $rw->i_t_id; ?></td>
                        <td><?php echo $rw->i_t_name; ?></td>
                        <td>
                          <a id="<?php echo $rw->i_t_id; ?>" class="pro_type_edit btn btn-sm btn-flat btn-success trash-multi" title="แก้ไข" data-toggle="modal" data-target="#pro_type_edit">
                            <i class="fa fa-wrench"></i> แก้ไข</a> 
                        </td>
                        <td>
                          <a id="<?php echo $rw->i_t_id; ?>" class="pro_type_delet btn btn-sm btn-flat btn-danger trash-multi"  title="ลบสถานะ">
  ...
  $(document).ready(function() {
        $(document).on('click','.pro_type_edit',function(evan) {
          ...
          var id = $(this).attr('id');
           $.ajax({
                method: "POST",
                url: "<?php echo site_url('Backend_control/type_ing_edit') ?>",
                dataType: 'json',
                data: {id:id},              
                        ...
                        $.each($result, function(i, v){
                          html += '<div class="content1"> '
                                    + '<p>'
                                      ...
                                      + '<input type="hidden" class="form-control" name="id" placeholder="รหัสส่วนผสม" value="'+ v["i_t_id"] +'">'
                                    + '</p> '
                                    + '<p>'
                                      + '<b>ประเภทของส่วนผสม <font color="red">*</font></b>  '
                                      + '<input type="text" class="pt form-control" name="i_name_type" placeholder="ชื่อประเภทของส่วนผสม" value="'+ v["i_t_name"] +'">'
                                    + '</p> '
          ...
          $(".th2").each(function(){
            ...
            if($(".th2").next().is(".require")==false){
                ...
                // alert ("ok");
                 $.ajax({
                    method: "POST",
                    url: "<?php echo site_url('Backend_control/type_ing_update') ?>",
                    dataType: 'json',
                ...
                success: function($result) {
                    ...
                    $.each($result, function(i, v){
                      // html += v["m_id"] ;
                      html += '<tr class="css_tr_data" id="'+ v["i_t_id"] +'">'
                                  + '<td>'
                                    + '<input name="checkdel[]" type="checkbox" class="css_data_item" id="'+ v["i_t_id"] +'" value="'+ v["i_t_id"] +'">' 
                                  + '</td>'
                                  + '<td>'+ v["i_t_id"] +'</td>'
                                  + '<td>'+ v["i_t_name"] +'</td>'
                                  + '<td>'
                                    + '<a id="'+ v["i_t_id"] +'" class="pro_type_edit btn btn-sm btn-flat btn-success trash-multi" title="แก้ไข" data-toggle="modal" data-target="#pro_type_edit">'
                                      + '<i class="fa fa-wrench"></i> แก้ไข</a> '
                                  + '</td>'
                                  + '<td>'
                                    + '<a id="'+ v["i_t_id"] +'" class="pro_type_delet btn btn-sm btn-flat btn-danger trash-multi"  title="ลบสถานะ">'
                                    + '<i class="fa fa-trash"></i> ลบสถานะ</a> '
                                  + '</td>'
                                + '</tr>';
  ...
  $(document).ready(function() {
        $(document).on('click','.pro_type_delet',function(evan) {
          ...
          var id = $(this).attr('id');
            $.ajax({
                method: "POST",
                url: "<?php echo site_url('Backend_control/type_ing_delet') ?>",
                dataType: 'json',
                data: {id:id},
                success: function($result) {
                        console.log($result);
  ...
  $(document).ready(function() {
        $(document).on('click','.type_delet_all',function(evan) {
          ...
          var id = $(this).attr('id');
           $.ajax({
                method: "POST",
                url: "<?php echo site_url('Backend_control/type_ing_delet_all') ?>",
                dataType: 'json',
          ...
          $(".pt").each(function(){
            ...
            if($(".pt").next().is(".require")==false){
                ...
                // alert ("ok");
                 $.ajax({
                    method: "POST",
                    url: "<?php echo site_url('Backend_control/type_ing_insert') ?>",
                    dataType: 'json',
