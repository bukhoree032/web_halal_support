<div class="content-wrapper">
    ...
    <section class="content-header">
      <h1>
        <b>ข้อมูลสถานที่ตรวจสอบ</b>
        <small></small>
      </h1>
      <ol class="breadcrumb">
        <li><a href="#"><i class="fa fa-dashboard"></i> หน้าหลัก</a></li>
        <li class="active">สถานที่ตรวจสอบทั้งหมด</li>
      </ol>
    </section>
          ...
          <div class="col-xs-12">
            <div class="box">
              <div class="box-header">
                <h3 class="box-title"><b>ข้อมูลสถานที่ตรวจสอบ</b></h3>
              </div>
              <!-- /.box-header -->
              <div class="box-body">
                    ...
                    <div class="box-tools pull-right">
                      <div class="btn-toolbar" role="toolbar" aria-label="Toolbar with button groups">
                        <div class="btn-group" role="group" aria-label="1 group">
                          <a href ="" class="btn btn-sm btn-success btn-flat box-add" title="เพิ่มสถานที่ตรวจสอบ" data-toggle="modal" 
                            data-target="#inves_insert">
                            <i class="fa fa-plus"></i> เพิ่มสถานที่ตรวจสอบ
                          </a>
                        </div>
                        <div class="btn-group" role="group" aria-label="2 group">
                          <a class="inves_delet_all btn btn-sm btn-flat btn-danger trash-multi bt1">
                              <i class="fa fa-trash"></i> ลบสถานที่ตรวจสอบที่เลือก</a>
                ...
                <table id="example1" class="table table-bordered table-striped">
                  <thead>
                    <tr class="css_tr_data">
                        ...
                        <th width='8%'>รหัสสถานที่</th>
                        <th width='60%'>ชื่อสถานที่ตรวจสอบ</th>
                        <th width='10%'>แก้ไข</th>
                        <th width='10%'>ลบสถานะ</th>
                    </tr>
                  </thead>
                  <tbody>
                    <?php foreach ($inves as $rw) { ?>
                      <tr class="css_tr_data">
                        <td>
                          <input name="checkdel[]" type="checkbox" class="css_data_item" id="<?php echo $rw->inves_id; ?>" value="<?php echo $rw->inves_id; ?>">
                        </td>
                        <td><?php echo $rw->inves_id; ?></td>
                        <td><?php echo $rw->inves_inspector; ?></td>
                        <td>
                          <a id="<?php echo $rw->inves_id; ?>" class="inves_edit btn btn-sm btn-flat btn-success trash-multi" title="แก้ไข" data-toggle="modal" data-target="#inves_edit">
                            <i class="fa fa-wrench"></i> แก้ไข</a> 
                        </td>
                        <td>
                          <a id="<?php echo $rw->inves_id; ?>" class="inves_delet btn btn-sm btn-flat btn-danger trash-multi"  title="ลบสถานะ">
...
<script type="text/javascript">
  $(document).ready(function() {
        $(document).on('click','.inves_edit',function(evan) {
          evan.preventDefault();
          // alert ($(this).attr('id'));
          var id = $(this).attr('id');
           $.ajax({
                method: "POST",
                url: "<?php echo site_url('Backend_control/inves_edit') ?>",
                dataType: 'json',
                data: {id:id},              
                        ...
                        $.each($result, function(i, v){
                          html += '<div class="content1"> '
                                    + '<p>'
                                      ...
                                      + '<input type="hidden" class="form-control" name="id" placeholder="รหัสส่วนผสม" value="'+ v["inves_id"] +'">'
                                    + '</p> '
                                    + '<p>'
                                      + '<b>ชื่อสถานที่ตรวจสอบ <font color="red">*</font></b>  '
                                      + '<input type="text" class="th2 form-control" name="name_inves" placeholder="ชื่อสถานที่ตรวจสอบ" value="'+ v["inves_inspector"] +'">'
                                    + '</p> '
...
<script type="text/javascript">
  $(document).ready(function() {
      ...
      $("#form_inves").submit(function(evan) {
      evan.preventDefault();
          ...
          $(".th2").each(function(){
            ...
            if($(".th2").next().is(".require")==false){
                ...
                // alert ("ok");
                 $.ajax({
                    method: "POST",
                    url: "<?php echo site_url('Backend_control/inves_update') ?>",
                    dataType: 'json',
                    data: $("#form_inves").serialize(),                
                success: function($result) {
                    console.log($result);
                    let html = "";
                    $.each($result, function(i, v){
...
<script type="text/javascript">
  $(document).ready(function() {
      //เรียกใช้ evnet submit
      $("#form_inves_insert").submit(function(evan) {
      evan.preventDefault();
          ...
          $(".th1").each(function(){
            ...
            if($(".th1").next().is(".require")==false){
                 $.ajax({
                    ...
                    dataType: 'json',
                    data: $("#form_inves_insert").serialize(),
                    success: function($result) {
                        console.log($result);
                          let html = "";
                              $.each($result, function(i, v){
...
<script type="text/javascript">
  $(document).ready(function() {
        $(document).on('click','.inves_delet',function(evan) {
          evan.preventDefault();
          // alert ($(this).attr('id'));
          var id = $(this).attr('id');
            $.ajax({
                method: "POST",
                url: "<?php echo site_url('Backend_control/inves_delet') ?>",
                dataType: 'json',
                data: {id:id},
                success: function($result) {
                        console.log($result);
...
<script type="text/javascript">
  $(document).ready(function() {
        $(document).on('click','.inves_delet_all',function(evan) {
          evan.preventDefault();
          // alert ($(this).attr('id'));
          var id = $(this).attr('id');
           $.ajax({
                method: "POST",
                url: "<?php echo site_url('Backend_control/inves_delet_all') ?>",
                dataType: 'json',
